...
async function setupClickHouse() {
  ...
  await clickhouse.query({
    query: `
      ...
      ENGINE = ReplicatedReplacingMergeTree('/clickhouse/tables/{shard}/events', '{replica}', processed_at)
      PARTITION BY toYYYYMM(event_time)
      ORDER BY (event_id, event_time)
      PRIMARY KEY (event_id)
  ...
  await clickhouse.query({
    query: `
      ...
      AS SELECT *
      FROM events_buffer
      GROUP BY event_id, event_type, event_name, event_time, user_id, session_id, page_name,
               element_id, referrer, device_type, platform, app_version, network_type,
               content_id, content_type, media_type, category, is_sponsored, age_hours,
               scroll_direction, position, duration_ms, scroll_depth, zoom_level,
               playback_position, is_autoplay, load_time_ms, connection_speed,
               city, country, timezone, latitude, longitude, location_accuracy,
               product_id, price, currency, transaction_id, promotion_code,
               raw_content, raw_interaction, raw_behavior, raw_performance,
               raw_commerce, raw_location, raw_experiments, raw_extras,
               processed_at
    `
  });
