...
function initSentry() {
  Sentry.init({
  dsn: process.env.SENTRY_DSN_TRANSFORM,
  ...
  Sentry.setTag("component", "kafka-consumer");
  Sentry.setTag("service", "transform");
  Sentry.logger.info('User triggered test log', { action: 'test_log' });
  // Verify Sentry is working by sending a test event
  Sentry.captureMessage("Transform service starting up", "info");
